...
async fn serve(listener: TcpListener, reply: Arc<String>) {
    loop {
...
{
    let arc_reply = Arc::new(reply.to_string());
    let first_task = serve(first, Arc::clone(&arc_reply));
    let second_task = serve(second, Arc::clone(&arc_reply));
